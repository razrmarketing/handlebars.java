...
public class MathHelper implements Helper<Object> {
    ...
    public CharSequence apply(final Object value, Options options) throws IOException, IllegalArgumentException {
        if (options.params.length >= 2) {
            ...
            String appliedResult = result.toString();

            if (options.params.length >= 3 && options.param(2) != null){
                NumberFormat numberFormat = new DecimalFormat(options.param(2).toString(), new DecimalFormatSymbols(Locale.US));
                appliedResult = numberFormat.format(result);
